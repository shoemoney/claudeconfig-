...
async function processMessage(msg) {
    ...
    if (msg.subject) {
        const sanitizedSubject = sanitizeMessageContent(msg.subject);
        content = `Subject: ${sanitizedSubject}\n${content}`;
    }
    const sanitizedSender = sanitizeMessageContent(msg.sender);
    const formattedMsg = {
    ...
    if (url) {
        const sanitizedUrl = sanitizeMessageContent(url);
        formattedMsg.url = sanitizedUrl;
        formattedMsg.content += `\n[URL: ${sanitizedUrl}]`;
    }
    return formattedMsg;
}
async function readMessages(phoneNumber, limit = 10) {
    try {
      ...
      LIMIT ${limit}
    `;
        const { stdout } = await retryOperation(() => execAsync(`sqlite3 -json "${process.env.HOME}/Library/Messages/chat.db" "${query}" ${phoneFormats.map(format => JSON.stringify(format)).join(' ')}`));
        if (!stdout.trim()) {
            console.error("No messages found in database for the given phone number");
            return [];
        }
        const messages = JSON.parse(stdout);
        const processedMessages = await Promise.all(messages
...
async function getUnreadMessages(limit = 10) {
    try {
      ...
      LIMIT ${limit}
    `;
        const { stdout } = await retryOperation(() => execAsync(`sqlite3 -json "${process.env.HOME}/Library/Messages/chat.db" "${query}"`));
        if (!stdout.trim()) {
